...
class QueryFilterFormat(BaseModel):
    ...
    id: int = Field(None, description="图片id (mini_id,id二选一)")
    types: int = Field(0, description="类型:0图片,1banner,2二维码")
...
def _generate_where_sql(request_data):
    ...
    if types:
        where_lst.append(f"types={types}")
    where_sql = (" where " if where_lst else "") + " and ".join('%s' % rec for rec in where_lst)
    return where_sql


def _get_pictures(request_data: QueryFilterFormat):
...
def _create_pictures(request_data: CreateFilterFormat):
    request_dict = request_data.dict()
    ...
    sob_handle = sob.sql_open(db_config)
    flag = sob.insert_Or_update_mysql_record_many_new(sob_handle,'wxapp_picture',[request_dict],hope_lst)
    sob.sql_close(sob_handle)
    if flag == -1:
        raise
    else:
...
def _delete_pictures(request_data: DeleteFilterFormat):
    sob_handle = sob.sql_open(db_config)
    cmd = f"delete from wxapp_picture where id={request_data.id}"
...
#【获取图片】
def get_pictures(request_data: QueryFilterFormat):
    try:
        res = _get_pictures(request_data)
...
#【创建修改图片】
def create_pictures(request_data: CreateFilterFormat):
    try:
        res = _create_pictures(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
...
#【删除图片】
def delete_pictures(request_data: DeleteFilterFormat):
    try:
        res = _delete_pictures(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
...
def upload_file(request: Request,file: UploadFile = File(...)):
    try:
        fname = f'./static/imgs/{str(time.time())}.png'
        with open(fname, "wb") as buffer:
            shutil.copyfileobj(file.file, buffer)
        url = f"{request.base_url }{fname}"
        response = format_response_data(url)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}