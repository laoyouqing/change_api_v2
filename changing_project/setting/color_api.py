...
class QueryFilterFormat(BaseModel):
    ...
    id: int = Field(None, description="配套颜色id (mini_id,id二选一)")


class CreateFilterFormat(BaseModel):
    mini_id: int = Field(..., description="小程序id")
    id: int = Field(None, description="id")
    title: str = Field(..., description="配套颜色项标示")
    values_json: str = Field(..., description="配套颜色内容(json格式)")


class DeleteFilterFormat(BaseModel):
    id: int = Field(..., description="配套颜色id")
...
def _get_color(request_data: QueryFilterFormat):
...
def _create_color(request_data: CreateFilterFormat):
    request_dict = request_data.dict()
    request_dict.update({"add_time":timer.get_now()})
    hope_lst = list(request_dict.keys())
    hope_lst.remove('add_time')
    sob_handle = sob.sql_open(db_config)
    flag = sob.insert_Or_update_mysql_record_many_new(sob_handle,'wxapp_color',[request_dict],hope_lst)
    sob.sql_close(sob_handle)
    if flag == -1:
        raise
    else:
...
def _delete_color(request_data: DeleteFilterFormat):
    sob_handle = sob.sql_open(db_config)
    cmd = f"delete from wxapp_color where id={request_data.id}"
...
#【获取配套颜色】
def get_color(request_data: QueryFilterFormat):
    try:
        res = _get_color(request_data)
...
#【创建修改配套颜色】
def create_color(request_data: CreateFilterFormat):
    try:
        res = _create_color(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
...
#【删除配套颜色】
def delete_color(request_data: DeleteFilterFormat):
    try:
        res = _delete_color(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
