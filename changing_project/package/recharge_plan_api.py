...
class QueryFilterFormat(BaseModel):
    ...
    id: int = Field(None, description="钱包充值套餐id (mini_id,id二选一)")
...
class CreateFilterFormat(BaseModel):
    ...
    gift_money: float = Field(..., description="赠送金额")
...
def _get_recharge_plan(request_data: QueryFilterFormat):
...
def _create_recharge_plan(request_data: CreateFilterFormat):
    request_dict = request_data.dict()
    ...
    sob_handle = sob.sql_open(db_config)
    flag = sob.insert_Or_update_mysql_record_many_new(sob_handle,'wxapp_recharge_plan',[request_dict],hope_lst)
    sob.sql_close(sob_handle)
    if flag == -1:
        raise
    else:
...
def _delete_recharge_plan(request_data: DeleteFilterFormat):
    sob_handle = sob.sql_open(db_config)
    cmd = f"delete from wxapp_recharge_plan where id={request_data.id}"
...
#【获取钱包充值套餐】
def get_recharge_plan(request_data: QueryFilterFormat):
    try:
        res = _get_recharge_plan(request_data)
...
#【创建修改钱包充值套餐】
def create_recharge_plan(request_data: CreateFilterFormat):
    try:
        res = _create_recharge_plan(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
...
#【删除钱包充值套餐】
def delete_recharge_plan(request_data: DeleteFilterFormat):
    try:
        res = _delete_recharge_plan(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
