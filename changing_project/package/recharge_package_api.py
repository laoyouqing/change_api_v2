...
class QueryFilterFormat(BaseModel):
    ...
    id: int = Field(None, description="充电套餐包id (mini_id,id二选一)")
...
class CreateFilterFormat(BaseModel):
    ...
    type: int = Field(..., description="套餐类型 (1:停车包 2:停车加充电包)")
    days: int = Field(..., description="天数")
    plan_name: str = Field(..., description="套餐名称")
    money: float = Field(..., description="充值金额")
    recharge_time: int = Field(..., description="充电时间")
    gift_recharge_time: int = Field(0, description="赠送小时")
    sort: int = Field(..., description="排序 (数字越小越靠前)")


class DeleteFilterFormat(BaseModel):
    id: int = Field(..., description="充电套餐包id")
...
def _generate_where_sql(request_data):
    ...
    if keyword:
        where_lst.append(f"plan_name like '%{sob.escape(keyword)}%'")
    where_sql = (" where " if where_lst else "") + " and ".join('%s' % rec for rec in where_lst)
    return where_sql


def _get_recharge_package(request_data: QueryFilterFormat):
...
def _create_recharge_package(request_data: CreateFilterFormat):
    request_dict = request_data.dict()
    ...
    sob_handle = sob.sql_open(db_config)
    flag = sob.insert_Or_update_mysql_record_many_new(sob_handle,'wxapp_recharge_package',[request_dict],hope_lst)
    sob.sql_close(sob_handle)
    if flag == -1:
        raise
    else:
...
def _delete_recharge_package(request_data: DeleteFilterFormat):
    sob_handle = sob.sql_open(db_config)
    cmd = f"delete from wxapp_recharge_package where id={request_data.id}"
...
#【获取充电套餐包】
def get_recharge_package(request_data: QueryFilterFormat):
    try:
        res = _get_recharge_package(request_data)
...
#【创建修改充电套餐包】
def create_recharge_package(request_data: CreateFilterFormat):
    try:
        res = _create_recharge_package(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
...
#【删除充电套餐包】
def delete_recharge_package(request_data: DeleteFilterFormat):
    try:
        res = _delete_recharge_package(request_data)
        response = format_response_data(res)
        return response
    except Exception as exc:
        return {"status": 400, "msg": exc.__str__()}
